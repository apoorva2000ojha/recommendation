window.onload = function () {
  ...
  function changeMovie() {
    fetch("https://48d6-49-36-99-240.in.ngrok.io/movie")
  ...
  document
    ...
    .addEventListener("submit", (e) => {
        ...
        if (ratings.length >= min) {
            ...
            fetch(`https://48d6-49-36-99-240.in.ngrok.io/recommend?ids=${i}&ratings=${r}`)
            .then((response) => response.json())
            .then((data) => {
                ...
                data.titles.forEach(film => {
                    $('#recommendations_box').append(`<p>${film}</p><div id="recommendations${i}"></div>`)
